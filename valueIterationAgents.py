...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount=0.9, iterations=100):
        ...
        for i in range(iterations):
            new_values = util.Counter()  # A Counter is a dict with default 0
            for state in mdp.getStates():
                max_value = -math.inf
                if not mdp.getPossibleActions(state):
                    new_values[state] = 0
                    continue
    ...
    def getPolicy(self, state):
        ...
        """
        # TODO add reward and discount to calculate of the policy
        ...
        for act in actions:
            cur_sum = sum([state_prob[1] * ((self.discount * self.values[state_prob[0]]) + self.mdp.getReward(state, act, state_prob[0]))
